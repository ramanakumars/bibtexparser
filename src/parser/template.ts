...
const get_groups_and_blocks = (text: string, start: number): Blocks => {
    ...
    for (const group of group_matches) {
        ...
        const group_obj: Group = { type: "group", text: group[0], blocks: group_block};
    ...
    for (const block of blocks) {
        ...
        if(block[0].indexOf("$group") !== -1) {
            ...
            const author_template = block[0].matchAll(/auth([sf])([0-9]+|a)?/g).next();
            var author_template_entry;
            if (author_template.value) {
                author_template_entry = { form: author_template.value[1], number: author_template.value[2] === "a" ? -1 : (isNumeric(author_template.value[2]) ? Number(author_template.value[2]) : 0)};
            }
            blocki = { type : "author", text: block[0], author_template: author_template_entry }
        } else {
